...
@section('content')
    ...
    <div class="container py-3">
        <a href="#" class="block text-[42px] text-black capitalize mb-2 text-center bold">{{ $blog->name }}</a>
        <div class="max-w-[720px] mx-auto w-full">
            <div class="flex items-end gap-2 mb-2">
                <img src="{{asset($blog->user?->image)}}" alt="" class="w-8 h-8">
                <a href="#" class="block text-black text-lg/5">{{$blog->user?->name}}</a>
            </div>

            <div
                class="flex items-center py-1 mb-6 border-t border-dashed border-slate-400 font-helvetica font-extralight">
                <div class="text-xs text-gray-500">Created at  {{ $blog->created_at->format('d M Y') }}
                </div>

                <a class="text-xs text-gray-500 uppercase ms-4" href="{{route('home.category',$blog->category?->slug)}}">{{$blog->category?->name}}</a>
            </div>

            <div class="max-w-[550px]">
                <img src="{{asset($blog->image)}}" alt="{{$blog->name}}"
                     class="aspect-[auto_700/533] w-full h-full object-cover object-center">
                <span class="text-xs text-gray-600 font-courier">{{$blog->user->name}}</span>
            </div>
            <!-- Social -->
            <div class="flex items-center gap-8 mt-3 mb-6">
                <a href="https://twitter.com/intent/tweet?url={{ urlencode(route('home.blog', $blog->slug)) }}&text={{ urlencode('Check out this blog post!') }}" target="_blank"
                ...
                <a target="_blank" href="mailto:?subject=Check%20this%20out&body={{ route('home.blog', $blog->slug) }}"
                   class="flex items-center gap-0.5 -space-y-0.5 text-sm text-gray-500 font-helvetica font-extralight">
                ...
                <a target="_blank" href="https://wa.me/?text={{ route('home.blog', $blog->slug) }}"
                   class="flex items-center gap-0.5 -space-y-0.5 text-sm text-gray-500 font-helvetica font-extralight">
                    <svg width="20" height="15" viewBox="0 0 20 15" fill="none" xmlns="http://www.w3.org/2000/svg"
                         style="width:1.25rem;height:0.9375rem;margin-bottom:0.125rem">
                        <rect width="20" height="14" transform="translate(0 0.740479)" fill="none"></rect>
                ...
                <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(route('home.blog', $blog->slug)) }}" target="_blank"
                   class="flex items-center gap-0.5 -space-y-0.5 text-sm text-gray-500 font-helvetica font-extralight">
            ...
            <!-- More content -->
            <div class="mt-8">
                <hr class="border-black mb-0.5">
                <hr class="border-black">
                <div class="flex items-center gap-3 mt-3 mb-6">
                    <img src="{{asset('homePage')}}/assets/images/&.png" alt="" class="w-6">
                    <p class="text-xl text-gray-900 uppercase">More from {{$website->name}}</p>
                </div>
                            ...
                            <a href="#"
                               class="block group odd:pl-3 even:pr-3 even:border-r even:border-dashed even:border-slate-400 after:content-[''] after:my-3 after:border-t after:border-dashed after:border-slate-400 after:w-full after:h-px relative after:absolute after:-bottom-6 after:inset-x-0 my-3 last:after:border-0 [&:nth-last-child(2)]:after:border-0">
                                <div class="flex items-center gap-2 pb-2 border-b border-dashed border-slate-400">
                                    <img src="{{asset($row->user->image)}}" alt="" class="w-10 h-10">
                                    <div>
                                ...
                                <div class="py-2">
                                    ...
                                    <div class="mb-2 text-base font-light group-hover:text-blue-700">{{$row->description}}
                                    </div>
                                </div>
                            </a>
                        </template>
